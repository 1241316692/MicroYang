...
import com.zuoyue.weiyang.common.UserManager;
...
//        if (roleTypeEnum == null) return null;
//        roleList.add(roleTypeEnum.getDesc());
//
//        // 为当前用户设置角色和权限
...
//        System.err.println("验证当前Subject时获取到token为" + ReflectionToStringBuilder.toString(token, ToStringStyle.MULTI_LINE_STYLE));
        ...
        if (user != null) {
//            if (!user.getActive()) throw new LockedAccountException("该账号已被冻结");

            ByteSource credentialsSalt = ByteSource.Util.bytes(user.getUsername());//这里的参数要给个唯一的;
            AuthenticationInfo authcInfo = new SimpleAuthenticationInfo(user.getUsername(), user.getPassword(), credentialsSalt, String.valueOf(user.getId()));
